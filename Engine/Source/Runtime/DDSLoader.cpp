...
namespace Nova
{
    ...
    Ref<Asset> DDSLoader::LoadFromStream(Stream& stream)
    {
        ...
        createInfo.device = renderDevice;
        createInfo.width = header.width;
        createInfo.height = header.height;
        createInfo.depth = header.depth;
        createInfo.mipCount = header.mipCount;
        createInfo.format = ConvertFormat(headerDXT10.format);
        createInfo.sampleCount = 1;
        createInfo.usageFlags = TextureUsageFlagBits::Default;

        Ref<Texture> texture = renderDevice->CreateTexture(createInfo);
