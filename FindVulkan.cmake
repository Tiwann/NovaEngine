...
``Vulkan::volk``
  ...
  Defined if SDK has the Slang library
...
``Vulkan_volk_LIBRARY``
  ...
  ``Vulkan_slang_LIBRARY``
  .. versionadded:: 3.30

  Path to the slang library.
...
_Vulkan_set_library_component_found(slang NO_WARNING)
...
if (Vulkan_FOUND)
    ...
    if ((Vulkan_slang_LIBRARY OR Vulkan_slang_DEBUG_LIBRARY) AND NOT TARGET Vulkan::slang)
        ...
        set_property(TARGET Vulkan::slang PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${Vulkan_INCLUDE_DIRS}")
        if (Vulkan_slang_LIBRARY)
            set_property(TARGET Vulkan::slang APPEND PROPERTY IMPORTED_CONFIGURATIONS Release)
            set_property(TARGET Vulkan::slang APPEND PROPERTY IMPORTED_LOCATION_RELEASE "${Vulkan_slang_LIBRARY}")
        endif ()
        if (Vulkan_slang_DEBUG_LIBRARY)
            set_property(TARGET Vulkan::slang APPEND PROPERTY IMPORTED_CONFIGURATIONS Debug)
            set_property(TARGET Vulkan::slang APPEND PROPERTY IMPORTED_LOCATION_DEBUG "${Vulkan_slang_DEBUG_LIBRARY}")
        endif ()
    endif ()
